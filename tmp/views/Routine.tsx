  ...
  return (
    <div className={`${className} flex flex-col gap-2 relative group/cell`}>
      <div
        ...
        dangerouslySetInnerHTML={readOnly ? { __html: initialHtml } : undefined}
      />
...
const Routine: React.FC<RoutineProps> = ({ isDarkMode }) => {
  ...
  const renderEditorToolbar = () => {
    if (view === 'viewer') {
      return (
        <div className="bg-slate-50 border-b-2 border-black p-4 flex justify-between items-center sticky top-0 z-[1100] shadow-sm">
           <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-black rounded-xl flex items-center justify-center">
                 <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
              </div>
              <h2 className="font-black text-xl text-slate-800 tracking-tighter uppercase">Modo de Visualização</h2>
           </div>
           <div className="flex gap-4">
              <button onClick={() => setView('editor')} className="bg-black text-white px-8 py-3 rounded-2xl font-black shadow-lg hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                Editar
              </button>
              <button onClick={() => setView('list')} className="bg-white border-2 border-slate-200 text-slate-500 px-8 py-3 rounded-2xl font-black hover:bg-slate-50 transition-all">Fechar</button>
           </div>
        </div>
      );
    }
    return (
      <div className="bg-slate-50 border-b-2 border-black p-4 flex flex-wrap gap-4 items-center sticky top-0 z-[1100] shadow-sm">
  ...
  const renderEditor = (isViewer: boolean = false) => {
    ...
    return (
      <div className="fixed inset-0 z-[1000] bg-white overflow-y-auto flex flex-col animate-fadeIn">
        ...
        <div className="p-8 flex items-center justify-between max-w-[98%] mx-auto w-full">
          <div className="flex items-center gap-4">
            <button onClick={() => changeWeek(Math.max(0, currentWeekIndex - 1))} disabled={currentWeekIndex === 0} className="w-12 h-12 border-2 border-black rounded-full flex items-center justify-center font-black disabled:opacity-20 hover:bg-slate-50 transition-all text-black shadow-sm">-</button>
            <div className="bg-black text-white px-8 py-3 rounded-2xl font-black text-xl shadow-lg">Semana {currentWeekIndex + 1}: {currentWeek.startDate} - {currentWeek.endDate}</div>
            <button onClick={() => changeWeek(Math.min(totalWeeks - 1, currentWeekIndex + 1))} disabled={currentWeekIndex === totalWeeks - 1} className="w-12 h-12 border-2 border-black rounded-full flex items-center justify-center font-black disabled:opacity-20 hover:bg-slate-50 transition-all text-black shadow-sm">+</button>
          </div>
        ...
        <div className={`flex-1 px-8 pb-12 ${transitionAnim}`}>
          <div className="bg-white border-2 border-black rounded-[2.5rem] overflow-hidden w-full shadow-2xl">
