...
type ViewState = 'entry' | 'list' | 'categories' | 'create_category' | 'editor' | 'calculator' | 'calculator_percent' | 'gym_menu' | 'gym_choice' | 'gym_now' | 'gym_train' | 'gym_subgroups' | 'gym_exercises' | 'gym_duration' | 'gym_save_day' | 'gym_active_session' | 'gym_weights' | 'gym_history' | 'gym_manage' | 'gym_edit_library' | 'gym_add_to_library' | 'finances' | 'nutrition_onboarding' | 'nutrition_onboarding_step2' | 'nutrition_dashboard';
...
interface NutritionProfile {
  ...
  gender: 'male' | 'female' | null; // Adicionado sexo
}
...
const Others: React.FC<OthersProps> = ({ isDarkMode }) => {
  ...
  const [nutriWeight, setNutriWeight] = useState(''); // New state for weight
  const [nutriGender, setNutriGender] = useState<'male' | 'female' | null>(null); // New state for gender
    ...
    if (view === 'editor') {
      ...
      if (lastInitializedNoteIdRef.current !== noteId) {
        ...
        if (titleInputRef.current) {
          titleInputRef.current.innerHTML = currentNote.title || '';
        }
    ...
    try {
      ...
      const generate = async () => {
        return await ai.models.generateContent({
          ...
          contents: 'Gere uma frase de motivação curta e poderosa para produtividade. Apenas a frase em português.',
        });
      };
      const response = await callWithRetry(generate);
      const text = response.text || "Sua disciplina de hoje é o sucesso de amanhã.";
      setMotivation(text);
  ...
  const handleSaveAction = () => {
    ...
    const htmlTitle = titleInputRef.current?.innerHTML || '';
  ...
  const finalizeSaveNote = (cat?: NoteCategory, overridenContent?: string, overridenTitle?: string) => {
    const noteToSave: Note = {
      ...(currentNote as Note),
      content: overridenContent !== undefined ? overridenContent : (editorRef.current?.innerHTML || ''),
      title: overridenTitle !== undefined ? overridenTitle : (titleInputRef.current?.innerHTML || ''),
      categoryId: cat?.id,
  ...
  const renderNutritionDashboard = () => (
    <div className="flex flex-col items-center justify-center py-20 animate-fadeIn text-center">
       ...
       <h2 className={`text-4xl font-black ${textColor} mb-4 uppercase tracking-tighter`}>Módulo Nutricional</h2>
       <p className="text-slate-400 font-bold max-w-md mb-8">Seus dados básicos estão salvos! Em breve você terá acesso a planos alimentares personalizados.</p>
       
       <div className="bg-white border-2 border-black rounded-3xl p-8 flex gap-8">
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Idade</span>
             <span className="text-3xl font-black text-slate-800">{nutritionProfile?.age}</span>
          </div>
          <div className="w-px bg-slate-200"></div>
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Altura</span>
             <span className="text-3xl font-black text-slate-800">{nutritionProfile?.height} <span className="text-sm">cm</span></span>
          </div>
          <div className="w-px bg-slate-200"></div> {/* Adicionado separador */}
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Peso</span>
             <span className="text-3xl font-black text-slate-800">{nutritionProfile?.weight} <span className="text-sm">kg</span></span>
          </div>
          <div className="w-px bg-slate-200"></div> {/* Adicionado separador */}
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Sexo</span>
             <span className="text-3xl font-black text-slate-800">{nutritionProfile?.gender === 'male' ? 'Homem' : 'Mulher'}</span>
          </div>
       </div>
    </div>
  );
  ...
  const finalizeNutritionProfile = () => {
    const profile = { age: nutriAge, height: nutriHeight, weight: nutriWeight, gender: nutriGender };
    setNutritionProfile(profile);
    ...
    return (
      <div className="animate-fadeIn max-w-md mx-auto flex flex-col items-center pt-10">
        ...
        <div className="w-full bg-white border-2 border-black rounded-[3rem] p-10 shadow-2xl flex flex-col gap-8">
           <div className="space-y-4">
              <label className="text-[10px] font-black text-[#4A69A2] uppercase tracking-widest pl-2 flex items-center gap-2">
              ...
              <input 
                type="number" 
                value={nutriAge} 
                onChange={(e) => setNutriAge(e.target.value)} 
                placeholder="Ex: 25" 
                className="w-full p-4 border-2 border-black rounded-xl font-black text-lg outline-none bg-slate-50 focus:bg-white transition-all text-slate-800" 
              />
           </div>
           ...
           <div className="space-y-4">
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  className={`flex-1 py-4 rounded-xl border-2 font-black text-sm uppercase transition-all ${nutriGender === 'male' ? 'bg-black text-white border-black' : 'bg-slate-50 text-slate-400 border-transparent hover:border-black'}`}
                >
                  Homem
                </button>
                <button 
                  onClick={() => setNutriGender('female')} 
                  className={`flex-1 py-4 rounded-xl border-2 font-black text-sm uppercase transition-all ${nutriGender === 'female' ? 'bg-black text-white border-black' : 'bg-slate-50 text-slate-400 border-transparent hover:border-black'}`}
                >
                  Mulher
                </button>
              </div>
  ...
  const renderNutritionOnboardingStep2 = () => {
    return (
      <div className="animate-fadeIn max-w-md mx-auto flex flex-col items-center pt-10">
        ...
        <div className="w-full bg-white border-2 border-black rounded-[3rem] p-10 shadow-2xl flex flex-col gap-8 text-center">
          <div className="w-24 h-24 bg-[#4A69A2]/10 rounded-full flex items-center justify-center mx-auto mb-8">
            <span className="text-4xl">🚀</span>
          </div>
          <h2 className="text-3xl font-black text-slate-800 uppercase tracking-tighter mb-4">Módulo de Nutrição</h2>
          <p className="text-slate-400 font-bold max-w-md">Mais perguntas virão aqui em breve para personalizar ainda mais o seu plano nutricional!</p>
          
          <button 
            onClick={finalizeNutritionProfile}
            className="w-full py-5 bg-black text-white rounded-2xl font-black uppercase tracking-widest shadow-xl hover:scale-105 active:scale-95 transition-all mt-8"
          >
            Salvar e Concluir
          </button>
        </div>
      ...
      {view === 'entry' && (
        <div className="flex flex-col items-center justify-center gap-12 py-12">
          <div className="flex flex-wrap justify-center gap-8 md:gap-10">
            ...
            <button onClick={() => setView('gym_menu')} className="group w-48 h-48 bg-white border-2 border-[#4A69A2] rounded-[3.5rem] shadow-2xl flex flex-col items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95 hover:bg-[#4A69A2]">
              <div className="p-5 bg-[#4A69A2] rounded-3xl mb-3 transition-all duration-300 group-hover:bg-white">
                    ...
                    getSortedNotes().map((note, idx) => (
                      <div key={note.id} onClick={() => { setCurrentNote(note); setView('editor'); }} className="p-6 rounded-[1.5rem] border-2 border-black shadow-md hover:scale-[1.01] cursor-pointer transition-all relative group flex flex-row items-center justify-between note-card-anim" style={{ backgroundColor: note.categoryColor, animationDelay: `${idx * 50}ms` }}>
                        <div className="flex flex-col flex-1 gap-1">
                          <h4 className="text-xl font-black text-slate-800 line-clamp-1" dangerouslySetInnerHTML={{ __html: note.title || 'Sem título' }}></h4>
