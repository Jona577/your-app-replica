...
const Study: React.FC<StudyProps> = ({ isDarkMode, onOpenDetail }) => {
  ...
  const handleReplaceMnemonic = async (id: string) => {
    ...
    try {
      ...
      const systemInstruction = `Você é o 'Mestre Mnemônico 2.0'. Gere APENAS UM objeto JSON para substituir um mnemônico que o usuário não gostou. 
...
RETORNE APENAS: { "type": string, "text": string, "explanation": string, "tip": string }`;
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Gere uma nova alternativa para: "${mnemonicInput}"`,
        config: { systemInstruction, responseMimeType: "application/json" }
      });

      const data = JSON.parse(response.text || "{}");
      setMnemonicResults(prev => prev.map(m => m.id === id ? { ...data, id: Date.now().toString() } : m));
    } catch (e) {
  ...
  const handleGenerateSimilar = async (parentMnemonic: MnemonicResult) => {
    ...
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const systemInstruction = `Você é o 'Mestre Mnemônico 2.0'. O usuário adorou este mnemônico: "${parentMnemonic.text}". 
...
RETORNE APENAS UM ARRAY JSON de objetos.`;
      ...
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Baseado no estilo de "${parentMnemonic.text}", gere 3 semelhantes para "${mnemonicInput}"`,
        config: { 
          systemInstruction, 
          // @ts-ignore
          ...
          responseSchema: {
            ...
            items: {
              ...
              required: ["type", "text", "explanation", "tip"]
            }
          }
        }
      });
      ...
      const newItems = data.map((item: any) => ({ ...item, id: Math.random().toString(36).substr(2, 9) }));
  ...
  const handleGenerateMnemonic = async () => {
    ...
    try {
      ...
      const systemInstruction = `Você é o 'Mestre Mnemônico 2.0', um cientista cognitivo e campeão mundial de memória.
...
Retorne um array de 5 objetos:
{
  ...
  "tip": string
      ...
      const generate = async () => {
        const response = await ai.models.generateContent({
          model: 'gemini-3-flash-preview',
          contents: `Desafio de Memorização: "${rawInput}"`,
          config: { 
            systemInstruction,
            // @ts-ignore
            responseMimeType: "application/json",
      ...
      setMnemonicResults(data.map((item: any) => ({ ...item, id: Math.random().toString(36).substr(2, 9) })));
      setErrorMsg(null);
    } catch (error: any) {
  ...
  const generateCuriosity = async (category: string) => {
    ...
    try {
      ...
      const textResponse = await callWithRetry(generateText);
      const data = JSON.parse(textResponse.text || "{}");

      // Now generate image with its own retry logic
      let imageUrl = "";
      try {
        const generateImage = async () => {
          return await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
            contents: `Ilustração digital 3D estilo Pixar, vibrant, NO TEXT: ${data.image_prompt}`,
            config: { imageConfig: { aspectRatio: "1:1" } }
          });
        };
        const imageResponse = await callWithRetry(generateImage);
      ...
      <div className="animate-fadeIn max-4xl mx-auto flex flex-col items-center py-10 h-full min-h-[500px]">
        ...
        <div className="h-96 w-full max-w-2xl perspective-1000 mb-12">
          ...
          >
            ...
            >
              <div className="text-xl font-medium text-slate-800 text-left leading-relaxed max-h-[90%] w-full overflow-y-auto custom-scrollbar prose prose-slate" dangerouslySetInnerHTML={{ __html: currentCard.flashcardBack || '' }} />
            </div>
          </div>
        </div>
            ...
            <div className="bg-white border-2 border-black rounded-[3rem] shadow-2xl relative overflow-hidden">
                ...
                <div className="bg-white p-12 min-h-[500px] flex flex-col rounded-b-[3rem]">
                    ...
                    <div 
                        ...
                        onInput={(e) => setFlashcardBack(e.currentTarget.innerHTML)}
